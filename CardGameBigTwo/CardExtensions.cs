namespace CardGame
{
    internal static class CardExtensions
    {
        ...
        internal static List<Card> GetFourOfAKind(this List<Card> cards, Card card, Func<Card, Card, bool> compareToCard = null)
        {
            // Group cards by rank
            var groupedByRank = cards.GroupBy(c => c.Rank);
            ...
            if (fourOfAKindGroup != null)
            {
                ...
                var fourOfAKind = fourOfAKindGroup.ToList().SortCards();

                if (compareToCard != null)
                {
                    var comparedCard = compareToCard(fourOfAKind[fourOfAKind.Count - 1], card);
                ...
                var restCards = cards.Except(fourOfAKind).ToList().SortCards();

                var groupedSingleByRank = restCards.GroupBy(c => c.Rank);

                // Find the group with single cards and get the one with the smallest rank
                var smallestSingleCardGroup = groupedSingleByRank.FirstOrDefault(group => group.Count() == 1);

                // Find the fifth card
                var fifthCard = smallestSingleCardGroup.First();

                var lastCardToAdd = fifthCard != null ? fifthCard : restCards[0];
                fourOfAKind.Add(lastCardToAdd);
                return fourOfAKind;
            }
        ...
        internal static List<Card> GetStraightFlush(this List<Card> hand, Card card, Func<Card, Card, bool> compareToCard = null)
        {
            // Group cards by suit
            var groupedBySuit = hand.GroupBy(c => c.Suit);
            ...
            foreach (var group in groupedBySuit)
            {
                ...
                for (int i = 0; i < sortedCards.Count - 1; i++)
                {
                    ...
                    if (i == sortedCards.Count - 2)
                    {
                        if (compareToCard == null || compareToCard(sortedCards.SortCards()[sortedCards.Count - 1], card))
                        {
                            return sortedCards;
                        }
                    }
        ...
        internal static List<Card> GetSingle(this List<Card> cards, Card specificCard, Func<Card, Card, bool> compareToCard = null)
        {
